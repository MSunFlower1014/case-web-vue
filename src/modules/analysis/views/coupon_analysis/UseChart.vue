<template>
  <div class="app-container">
    <el-row :gutter="20"
      class="chartNum">
      <el-col :span="3"
        class="fb ft_18">{{subTitie}}数:{{useNum}}</el-col>
      <el-col :span="3">较昨天:
...
export default {
  ...
  methods: {
    ...
    handleQueryLineData() {
      if (this.timeCheck()) {
        this.queryUseAnalysisData('line')
      } else {
        return false
      }
    },
    queryUseAnalysisData(type = 'both') {
      let self = this
      this.queryParam.couponId = this.couponId
      couponAnalyService
        .queryUseAnalysisData(this.queryParam)
        .then(rsp => {
